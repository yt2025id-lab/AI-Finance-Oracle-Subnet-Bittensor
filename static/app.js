// ================================================================
// AI Finance Oracle Subnet — 3 Demo Scripts with Miner/Validator Detail
// ================================================================
    ...
    el.innerHTML = `
        ...
        <div class="result-topbar">
            <div class="result-topbar-left">
                ...
                <span style="font-size:13px;color:var(--text-secondary)">${syn.target_asset} | ${syn.time_horizon_minutes}min</span>
            </div>
        ...
        <div class="challenge-info">
            <div class="challenge-row">
                <span class="challenge-label">Asset</span><span class="challenge-value">${syn.target_asset}</span>
            </div>
            <div class="challenge-row">
                <span class="challenge-label">Exchange</span><span class="challenge-value">${syn.exchange || 'Any'}</span>
            </div>
            <div class="challenge-row">
                <span class="challenge-label">Volatility</span><span class="challenge-value">${(cond.volatility || 'normal')}</span>
            </div>
            <div class="challenge-row">
                <span class="challenge-label">Trend</span><span class="challenge-value">${(cond.trend || 'neutral')}</span>
            </div>
            <div class="challenge-row">
                <span class="challenge-label">Macro Event</span><span class="challenge-value">${(cond.macro_event || 'none').replace(/_/g, ' ')}</span>
            </div>
            ${data.ground_truth ? `
            <div class="challenge-row gt">
                <span class="challenge-label">Actual Price</span><span class="challenge-value gt-val">$${data.ground_truth.actual_price ? data.ground_truth.actual_price.toLocaleString() : '—'}</span>
            </div>
            <div class="challenge-row gt">
                <span class="challenge-label">Actual Direction</span><span class="challenge-value gt-val">${data.ground_truth.actual_direction || '—'} (${data.ground_truth.price_change_pct > 0 ? '+' : ''}${data.ground_truth.price_change_pct}%)</span>
            </div>
            ` : ''}
        </div>
                    ...
                    <div class="miner-card ${i === 0 ? 'miner-top' : ''}">
                        ...
                        <div class="miner-metrics">
                            <div class="miner-metric">
                                <span class="mm-label">Price</span>
                                <span class="mm-value">$${m.predicted_price ? m.predicted_price.toLocaleString() : '—'}</span>
                            </div>
                            <div class="miner-metric">
                                <span class="mm-label">Direction</span>
                                <span class="mm-value ${m.predicted_direction === 'bullish' ? 'mm-green' : 'mm-red'}">${m.predicted_direction}</span>
                            </div>
                            <div class="miner-metric">
                                <span class="mm-label">Sentiment</span>
                                <span class="mm-value ${m.sentiment_score > 0 ? 'mm-green' : 'mm-red'}">${m.sentiment_score > 0 ? '+' : ''}${m.sentiment_score}</span>
                            </div>
                            <div class="miner-metric">
...
function renderError(message) {
    ...
    el.innerHTML = `
        ...
        <div style="padding:20px;text-align:center;color:var(--text-muted)">
            ...
            <p>Make sure the API server is running: <code style="color:var(--gold-300)">uvicorn main:app --reload</code></p>
        </div>
        <div class="result-footer">
